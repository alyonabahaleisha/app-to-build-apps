...
// ─── T-0010-001 — Happy ───────────────────────────────────────────────────
// Bumped from v0.2.1 → v0.3.0 per V0 actions surface fix (increment verb + valueBinding).
it('T-0010-001: PROMPT_VERSION === "v0.3.0"', () => {
  expect(PROMPT_VERSION).toBe('v0.3.0')
})
...
it('T-0010-003: PROMPT_VERSION is exported as const (literal type preserved)', () => {
  // TypeScript const assertion ensures the literal type "v0.3.0" not widened to string.
  // At runtime we verify the value is a string matching the literal — TS enforces the rest.
  const version: 'v0.3.0' = PROMPT_VERSION
