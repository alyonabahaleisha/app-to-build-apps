...
Last updated: 2026-05-10 (updated by Ellis — ADR-0008, ADR-0010, ADR-0011 rows added)
...
| 0004 | Plan → Build Generation Pipeline                                   | **Superseded by ADR-0007** (2026-05-09) | llm, a2ui, tests                 | Two-stage generation: Haiku 4.5 planner (archetype + screens + nav) feeds Sonnet 4.6 builder (A2UI spec or RFC 6902 patch). Plan persisted as `plan_json` on `project_versions`. M1 single-call path preserved as fallback. Feature-flagged percent rollout. **Superseded:** Planner stage, `/edit` route, `pipeline.ts`, `serializePlan.ts`, `patchValidation.ts`, and `specValidation.ts` all deleted in ADR-0007 Step 6. Telemetry whitelist + eval-mode carry forward as `EVAL_MODE`. `plan_json` column kept for legacy alpha reads only. |
...
| 0007 | LLM Generation Pipeline Cutover to V0                              | Proposed (PRs 1–4 implemented; pending kill review) | llm, prompts, generation, v0, tests | Single-call Sonnet pipeline replacing ADR-0004 two-stage. `tool_choice: 'auto'` over 2 tools (`produce_app_spec` + `out_of_scope`) — documented deviation for V0.5 capability detection. Edit pipeline deleted; re-prompt-to-edit hits `/generate`. Deletes planner, pipeline.ts, edit route. New `out_of_scope_intent` table + endpoint. Telemetry whitelist + EVAL_MODE rename. 7 steps across 4 PRs. PR 4 (Step 7 eval harness, `f970acc`) cleared after Cal ACCEPT on Roz NOTE-2; deviation D-0007-01 documented (T-0007-160..166 + T-0007-170..172 as arithmetic mirrors / type guards; live CI as functional gate; runtime `writeResults` assertion follow-up non-blocking). Awaiting M1 cutover kill review. |
| 0008 | Universal Links — Canvas V0 Clone Share Flow                        | Proposed | mobile-shell, db, infra, v0     | `https://canvas.app/m/{share_id}/clone` Universal Link share + clone flow. AASA file hosting, pending-intent survival across SIWA round-trip (ADR-0011 surface), `view`/`remix` reserved-mode graceful handling. 7 steps, ~168 T-IDs. BLOCKED on ADR-0011 `mini_app` schema rename landing first. Web install-gate page deferred to V0.5. |
...
| 0010 | V0 Prompt Engineering + Eval-Grading Loop                           | Proposed (PR 1 implemented) | llm, prompts, generation, v0, tests | Closes the human-in-the-loop gap between structural eval and visual quality. 7 steps: PROMPT_VERSION semver gate, `ARCHETYPE_PROMPTS` labeled harness, per-PR baseline regression, Sable weekly grading rubric (primary score + dimension tags), `capture-eval-screenshots.sh` (Step 6 deep-link now `appcreator://devmenu/load-spec?fixture=<name>` per ADR-0011). 161 T-IDs. PR 1 (system prompt v0.1.0) committed at `c5d7635`. |
| 0011 | Mobile V0 Shells + `mini_app` Schema Rename                         | Proposed | mobile-shell, db, auth, v0      | The largest V0 ADR: 14 steps, 349 T-IDs, 3-phase rollout (3–4 weeks / 5–10 PRs). Renames `projects`→`mini_app`, `project_versions`→`mini_app_version` (Option A: in-place, no prod data); `sync_mode` Option 1 (no Postgres DEFAULT; COALESCE to `'local'` for grandfathered rows; Drizzle `.default('cloud-private')` for new). 2-tab structure (Library + Create); Settings as Gorhom Bottom Sheet; Run as pushed mode; custom `ShellLayout` for VoiceOver focus order. SIWA deferred to ADR-0013. Declares `appcreator://devmenu/load-spec?fixture=<name>` AUTHORITATIVE (consumed by ADR-0010 Step 6 + ADR-0008 pending-intent). R1→R2→R3 review cycle (Roz-approved). |
