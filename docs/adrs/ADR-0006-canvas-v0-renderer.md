...
Each exception is a documented architectural decision. Adding a 7th
...
| 6 | Carousel autoplay setInterval | `src/v0/components/lists/Carousel.tsx` | `setInterval` for time-based auto-advance has no synchronous equivalent in React. Timer must be started on mount/`shouldAutoplay`-true and cleared on unmount/`shouldAutoplay`-false. Strictly controlled via `shouldAutoplay = node.autoplay && !reducedMotion` guard so reduced-motion respect is at the gating layer, not inside the effect. | ADR-0009 Step 4 (V1 Phase 1 Lists & Data tier) |

The ESLint rule's `ignores` list reflects these 6 exceptions. Adding
