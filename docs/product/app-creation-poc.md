...
5. User types: _"Make me a morning routine tracker — three habits I can check off, with a streak counter."_ Taps Send.
...
9. User taps the saved app → AppRunner opens with the _same_ rendered UI, identical to first creation.
...
- **Two distinct UI layers** to design — don't blur them:
  ...
  2. **A2UI renderer components** (Heading, Text, Button, etc.) — your work _also_, but these are constrained to the 10-component catalog. The visual treatment of a `Button` here is the _only_ button style users will see in their generated apps. Pick carefully.
