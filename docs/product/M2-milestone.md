...
> Milestone-level brief, not a feature spec. Owns the _measurable target_ and
> the _kill criteria_. Per-feature specs (multi-screen renderer, navigation
...
correctness. M1 proved we _can_ generate; M2 proves the output is _good
enough that users come back and share_.
...
not directionally on track at that point, M2 ships as a _learning_ (H5
...
- M1 proved generation works in eval; we still don't know whether anyone
  _uses what they generate_. Today's <10% second-session rate is the wall.
...
| **H5 turns out to be false** (users abandon even beautiful generations)                 | **Most valuable risk to surface.** Week 7 kill-criteria review. M2 declared a _learning_ rather than a ship. Sponsor briefed on this possibility at kickoff                          | Rubric-top quartile second-session return <25% at week 7 → present disconfirmation as the deliverable            |
